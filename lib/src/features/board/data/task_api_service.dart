...
class TaskApiService {
  ...
  TaskApiService({
    this.baseUrl = const String.fromEnvironment('API_BASE_URL', defaultValue: 'https://api.dmaas.trycloudflare.com'),
  ...
  Future<List<TaskComment>> fetchComments(String taskId) async {
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> data = jsonDecode(response.body);
        return data.map((json) => TaskComment.fromJson(json)).toList();
      } else {
